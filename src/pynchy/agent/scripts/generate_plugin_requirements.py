...
def generate_requirements(output_path: Path, config_path: Path) -> int:
    ...
    if config_path.exists():
        ...
        _stdout_line(f"Config not found at {config_path}; writing empty plugin requirements file")

    header = [
        "# Auto-generated by src/pynchy/agent/scripts/generate_plugin_requirements.py",
        "# Contains config-managed pynchy plugins to install into container image.",
    ]
    content = "\n".join([*header, "", *lines, ""])
    output_path.write_text(content, encoding="utf-8")

    _stdout_line(f"Wrote {len(lines)} plugin requirement(s) to {output_path}")
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="src/pynchy/agent/requirements-plugins.txt",
