...
_PREK_GENERATED_MARKER = "File generated by prek:"
_PRE_COMMIT_GENERATED_MARKER = "File generated by pre-commit:"
_DEFAULT_PRE_COMMIT_CONFIG_ARG = "--config=.pre-commit-config.yaml"
...
def _shared_git_hooks_dir(repo_root: Path) -> Path | None:
    """Resolve the hooks directory shared by a checkout and its worktrees."""
    ...
    if dot_git.is_dir():
        ...
        if not git_dir_line.startswith(prefix):
            return None
        git_dir = Path(git_dir_line.removeprefix(prefix).strip())
...
def _has_obsolete_pre_commit_migration_hook(repo_root: Path) -> bool:
    """Return whether Prek is chaining an absent default pre-commit config."""
    if any(
        (repo_root / config_name).exists()
        for config_name in (".pre-commit-config.yaml", ".pre-commit-config.yml")
    ):
        return False

    hooks_dir = _shared_git_hooks_dir(repo_root)
    if hooks_dir is None:
        return False
    try:
        active_hook = (hooks_dir / "pre-commit").read_text(encoding="utf-8")
        legacy_hook = (hooks_dir / "pre-commit.legacy").read_text(encoding="utf-8")
    except (OSError, UnicodeError):
        return False
    return (
        _PREK_GENERATED_MARKER in active_hook
        and _PRE_COMMIT_GENERATED_MARKER in legacy_hook
        and _DEFAULT_PRE_COMMIT_CONFIG_ARG in legacy_hook
    )
...
def _retire_obsolete_pre_commit_migration_hook(repo_root: Path, runner: str) -> bool:
    if runner != "prek" or not _has_obsolete_pre_commit_migration_hook(repo_root):
        return True
    overwrite = _run_hook_installer(repo_root, runner, overwrite=True)
    if overwrite.returncode != 0:
        logger.warning(
            "Obsolete pre-commit migration hook cleanup failed (workspace unaffected)",
            repo=str(repo_root),
            stderr=overwrite.stderr.strip(),
        )
        return False
    logger.info("Obsolete pre-commit migration hook retired", repo=str(repo_root))
    return True
...
def install_repo_hooks(repo_root: Path) -> None:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            return
        if not _retire_obsolete_pre_commit_migration_hook(repo_root, runner):
            return
