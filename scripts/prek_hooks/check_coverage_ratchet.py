...
def measured_ratchet() -> Decimal:
    """Return the coverage floor represented by the current coverage data."""
    coverage = Coverage()
    coverage.load()
    total = coverage.report(file=io.StringIO())
    precision = coverage.config.precision
    measured = Decimal(f"{total:.{precision}f}")
    if measured == 100 and total != 100:
        return Decimal(100) - Decimal(1).scaleb(-precision)
    return measured
