...
2. **[FIXED] IDLE_TIMEOUT == CONTAINER_TIMEOUT** — both defaulted to 30 min, so an idle container was pinned for the full hard-timeout window with no room for graceful `_close` hibernation ahead of the `docker stop`→SIGKILL (137) teardown. Fixed: `container.idle_timeout_ms` lowered to 15 min (`config/models.py`), kept well under the 30-min `timeout_ms` safety net so finished containers hibernate cooperatively first.

3. **[NOT A BUG — stale TS note] Cursor advanced before agent succeeds** — the TS symbols (`processGroupMessages`/`lastAgentTimestamp`) don't map to the Python port, which already handles this correctly. The persisted cursor (`last_agent_timestamp`) advances only *after* the agent completes (`messaging/pipeline.py:502`), never before; on timeout/mid-run death `run_agent` returns `"error"` and, if no output reached the user, the cursor is left untouched so the messages retry (`pipeline.py:491-500`). `advance_cursor` rolls back on a save failure (`pipeline.py:326-343`), and `recover_pending_messages` re-enqueues boundary messages after a restart (`startup_handler.py:84-114`). No fix needed.
