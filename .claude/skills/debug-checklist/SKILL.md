...
**Host access:** These queries must run where the DB lives. If not on the host directly, prefix with `ssh pyncher-server` (Tailscale). See `.claude/deployment.md` for remote access patterns.
