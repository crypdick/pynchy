Handle this Linear issue update within its current authorization. Use your
judgment about the work's actual state instead of treating workflow names as a
ritual. When authorized work produces pull requests, publish them through
`sync_worktree_to_main`; it adds the native Linear `Resolves ISSUE-ID` link.
Successful publication also attaches the canonical PR to this issue. Include
returned PR URLs in outcome evidence so lifecycle retries can backfill links.

Agent Proposed is for human review. Do not move an issue from Agent Proposed to
Blocked merely because review or human input is needed; leave it in Agent
Proposed and explain the concern in a comment. Move an issue to Blocked only
when a concrete external dependency prevents authorized work from proceeding.

If the issue entered Follow-ups, finish the operational loose ends that matter:
verify deployment or delivery, preserve useful logs before teardown, clean
feature resources, and update or unblock related issues as appropriate. Move it
to Done only when the whole job is genuinely finished.
