...
| `prompts.*`, `pipelines.*`, `workspaces.*.soul`, `workspaces.*.pipeline`, and prompt file content | Affected-session retirement | Preflight rereads the selection, but warm IPC cannot replace `AgentCoreConfig.system_prompt_append`. Retire the worker and provider session so one conversation never resumes under a different system prompt. |
