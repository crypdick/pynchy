...
validated PR URL and exact in-flight execution. An `Awaiting Review` move also
idempotently backfills every GitHub pull request in effective `evidence_refs`
before changing issue state. Returning a final response doesn't imply a state
transition.
