...
Third-party plugins run on the host machine with full process-level access. A malicious plugin could exfiltrate secrets, execute arbitrary commands, or compromise the system during import. The plugin scanner mitigates this by running an LLM-based security audit on every new plugin revision before it is installed.

This page explains how the scanner works and how to configure it.

## How It Works

The scanner fits into the startup sequence. On every boot:
...
User messages are blocked until the audit completes — channels don't connect until after step 4.

## What the Scanner Looks For

The scanning agent receives the plugin source code (read-only) inside its container and uses bash to explore it. It checks for:
...
The verdict and reasoning are stored in the cache. A failed plugin stays blocked until its git SHA changes (i.e., the author pushes a fix).
...
Verdicts are stored in `~/.config/pynchy/plugins/verifications.db` (SQLite). Each row is keyed by `(plugin_name, git_sha)`, so updating a plugin to a new commit triggers a re-scan automatically.
...
- The scanner is an LLM — it can miss novel attack vectors or false-positive on legitimate patterns. It is a defense-in-depth layer, not a guarantee.
- Each scan spawns a full container agent, which takes 30–120 seconds depending on plugin size and LLM latency.
- The restart adds one extra boot cycle when new plugins are first installed.
