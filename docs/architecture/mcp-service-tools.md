...
Admin workspaces bypass all policy gates. Non-admin workspaces are gated by tool trust declarations — see [Service Trust Policy](security.md#5-service-trust-policy-lethal-trifecta-defenses) for the architecture and [Tool Trust](../usage/security.md) for configuration.
