...
│  │    • mcp__pynchy__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── pynchy.log               # Host stdout
│   └── pynchy.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.pynchy.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'pynchy-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your pynchy installation
...
Pynchy uses a hierarchical memory system based on CLAUDE.md files.
...
Pynchy has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__pynchy__schedule_task]
        {
...
The `pynchy` MCP server is created dynamically per agent call with the current group's context.
...
Pynchy runs as a single macOS launchd service.

### Startup Sequence

When Pynchy starts, it:
1. **Ensures Apple Container system is running** - Automatically starts it if needed; kills orphaned Pynchy containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/pynchy.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/pynchy.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.pynchy.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep pynchy` |
| "Claude Code process exited with code 1" | Apple Container failed to start | Check logs; Pynchy auto-starts container system but may fail |
...
- `logs/pynchy.log` - stdout
- `logs/pynchy.error.log` - stderr
