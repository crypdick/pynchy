...
@pytest.mark.asyncio
async def test_startup_failure_stops_gateway_when_mcp_cleanup_fails() -> None:
    mcp_manager = MagicMock()
    ...
    set_gateway.assert_called_once_with(None)


@pytest.mark.asyncio
async def test_startup_failure_retains_gateway_when_its_cleanup_fails() -> None:
    mcp_manager = MagicMock()
    mcp_manager.stop_all = AsyncMock()
    gateway = MagicMock()
