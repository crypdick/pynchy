...
from pynchy.plugin.builtin_tailscale import _TailscaleTunnel
...
class TestTailscaleTunnel:
    ...
    def test_is_available_found(self):
        with patch(
            "pynchy.plugin.builtin_tailscale.shutil.which",
            return_value="/usr/bin/tailscale",
        ):
            assert _TailscaleTunnel().is_available()

    def test_is_available_not_found(self):
        with patch("pynchy.plugin.builtin_tailscale.shutil.which", return_value=None):
    ...
    def test_is_connected_running(self):
        ...
        with patch("pynchy.plugin.builtin_tailscale.subprocess.run", return_value=mock_result):
            t = _TailscaleTunnel()
    ...
    def test_is_connected_not_installed(self):
        with patch(
            "pynchy.plugin.builtin_tailscale.subprocess.run",
