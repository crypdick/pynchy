...
class TestDetectMainBranch:
    def test_parses_branch_from_symbolic_ref(self):
        with patch("pynchy.git_utils.run_git", return_value=_ok("refs/remotes/origin/main\n")):
            assert detect_main_branch() == "main"

    def test_parses_non_standard_branch_name(self):
        with patch("pynchy.git_utils.run_git", return_value=_ok("refs/remotes/origin/master\n")):
            assert detect_main_branch() == "master"

    def test_falls_back_to_main_on_failure(self):
        with patch("pynchy.git_utils.run_git", return_value=_fail()):
            assert detect_main_branch() == "main"
...
class TestGetHeadSha:
    def test_returns_sha_on_success(self):
        with patch("pynchy.git_utils.run_git", return_value=_ok("abc123\n")):
    ...
    def test_returns_unknown_on_exception(self):
        with patch("pynchy.git_utils.run_git", side_effect=OSError("no git")):
            assert get_head_sha() == "unknown"
...
class TestIsRepoDirty:
    def test_dirty_when_porcelain_has_output(self):
        with patch("pynchy.git_utils.run_git", return_value=_ok(" M file.py\n")):
            assert is_repo_dirty() is True

    def test_clean_when_porcelain_is_empty(self):
        with patch("pynchy.git_utils.run_git", return_value=_ok("")):
...
class TestCountUnpushedCommits:
    def test_returns_count_on_success(self):
        with patch("pynchy.git_utils.run_git") as mock:
            mock.side_effect = [
...
class TestFilesChangedBetween:
    def test_returns_true_when_files_changed(self):
        with patch("pynchy.git_utils.run_git", return_value=_ok("src/app.py\n")):
