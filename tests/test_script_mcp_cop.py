...
def _make_settings_with_mcp(
    ...
    *,
    ...
    """Create fake Settings with an MCP server entry and safe service trust."""
    mock_s = MagicMock()
    mcp_mock = MagicMock()
    mcp_mock.type = mcp_type
    mock_s.mcp_servers = {tool_name: mcp_mock}
    mock_s.services = {}
    # Workspace with all-safe service trust so policy lets it through
    mock_s.workspaces = {
        "test-ws": WorkspaceConfig(
            name="test",
            security=WorkspaceSecurityTomlConfig(
                services={
                    tool_name: ServiceTrustTomlConfig(
                        public_source=False,
                        secret_data=False,
                        public_sink=False,
