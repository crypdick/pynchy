...
def _write_prek_migration_hooks(tmp_path: Path, *, legacy_hook: str) -> None:
    hooks_dir = tmp_path / ".git" / "hooks"
    hooks_dir.mkdir(parents=True)
    (hooks_dir / "pre-commit").write_text("# File generated by prek:\n")
    (hooks_dir / "pre-commit.legacy").write_text(legacy_hook)


def test_install_repo_hooks_retires_obsolete_pre_commit_migration_hook(tmp_path: Path) -> None:
    (tmp_path / "prek.toml").write_text("")
    _write_prek_migration_hooks(
        tmp_path,
        legacy_hook=(
    ...
    assert [call.args[0] for call in run.call_args_list] == [
        ...
        ["uv", "tool", "run", "prek", "install", "--overwrite"],
    ]


def test_install_repo_hooks_preserves_unrelated_legacy_hook(tmp_path: Path) -> None:
    (tmp_path / "prek.toml").write_text("")
    _write_prek_migration_hooks(
        tmp_path,
        legacy_hook="#!/bin/sh\nexec ./scripts/custom-check\n",
    )
...
def test_install_repo_hooks_keeps_pre_commit_migration_when_config_exists(tmp_path: Path) -> None:
    (tmp_path / "prek.toml").write_text("")
    (tmp_path / ".pre-commit-config.yaml").write_text("")
    _write_prek_migration_hooks(
        tmp_path,
...
def test_install_repo_hooks_finds_shared_hooks_from_linked_worktree(tmp_path: Path) -> None:
    ...
    hooks_dir.mkdir()
    (hooks_dir / "pre-commit").write_text("# File generated by prek:\n")
    (hooks_dir / "pre-commit.legacy").write_text(
        "# File generated by pre-commit:\n"
        "ARGS=(hook-impl --config=.pre-commit-config.yaml --hook-type=pre-commit)\n"
    )
