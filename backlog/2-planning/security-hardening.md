...
This project is broken into 7 sequential sub-plans:

### [Step 1: Workspace Security Profiles](security-hardening-1-profiles.md)
**Scope:** Config schema and validation for security profiles
**Time:** 2-3 hours
**Dependencies:** None

Define the security model: which tools each workspace can access and their risk tiers. Pure configuration layer - no service integrations yet.

### [Step 2: MCP Tools & Basic Policy](security-hardening-2-mcp-policy.md)
**Scope:** New IPC MCP tools + policy middleware
**Time:** 4-6 hours
**Dependencies:** Step 1

Add MCP tools for email, calendar, passwords. Implement policy enforcement middleware that gates tool execution based on security profiles. Tools return mock responses (real service integrations come in Steps 3-5).
...
**Time:** 3-4 hours
**Dependencies:** Steps 1-2, (Step 6 recommended)

Implement password manager adapter using 1Password CLI. Agents can search and retrieve passwords (with human approval).

**IMPORTANT:** Step 6 (Human Approval Gate) is critical for this feature to be safe.
...
- **Steps 1-6 (required):** 22-29 hours (3-4 full work days)
...
4. **Audit trail** - All actions logged with timestamps and approvals
5. **Workspace isolation** - Each workspace has independent security profile
