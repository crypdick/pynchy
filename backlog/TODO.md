...
- **Internal event queue for channel message ingestion** — Channel adapters currently call `asyncio.ensure_future(_on_inbound(...))` fire-and-forget. If Socket Mode drops an event, it's gone until the 10s periodic catch-up recovers it. Better design: channel adapters push to an internal `asyncio.Queue`, a single consumer drains it (store, cross-post, enqueue). This gives backpressure, ordering guarantees, and a single ingestion code path. Optionally, the Slack plugin could also switch from Socket Mode (WebSocket, no delivery guarantees) to HTTP Events API (built-in 3x retry) — same Bolt app, same event handlers, just a transport swap in `connect()` (requires public HTTPS endpoint, which Tailscale already provides).
